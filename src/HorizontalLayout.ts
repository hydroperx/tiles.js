...
export class HorizontalLayout extends Layout {
  constructor($: Tiles, max_width: number, max_height: number) {
  ...
  override snap_to_grid(tile: string, el: HTMLElement): void {
    ...
    const prev_group = this.groups.find(
      (g) => !!g.tiles.find((t) => t.id == tile),
    );
    const tile_state = this.$._state.tiles.get(tile);
    if (!tile_state) return;
    const tile_data =
      (prev_group ? prev_group.tiles.find((t) => t.id == tile) : null) ??
      new Tile(
        tile,
        el as HTMLButtonElement,
        tile_state.x,
        tile_state.y,
        get_size_width_small(tile_state.size),
        get_size_height_small(tile_state.size),
      );

    let x = this.offset_x_to_x(dropped_x),
      y = this.offset_y_to_y(dropped_y);

    if (!x) x = this.forced_offset_x_to_x(dropped_x);
    ...
    if (x && y) {
      if (x.group == "") {
        ...
        if (
          ...
          if (prev_group)
            prev_group.remove(tile), prev_group.auto_self_removal();
          tile_data.x = x.x;
          tile_data.y = y.y;
        ...
        if (prev_group) prev_group.remove(tile);
        if (
          new_group!.is_area_available(
            x.x,
            y.y,
          ...
          new_group!.add(tile_data);
        } else if (prev_group) prev_group.add(tile_data);
      }
    }
      ...
      if (x.group == "") {
        ...
        if (
          ...
          return {
            ...
            w:
              tile_data.width * this.$._small_size +
              (tile_data.width - 1) * this.$._tile_gap,
            h:
              tile_data.height * this.$._small_size +
              (tile_data.height - 1) * this.$._tile_gap,
          };
        }
      } else {
        const new_group = this.groups.find((g) => g.id == x.group);
  ...
  override offset_x_to_x(x: number): { group: string; x: number } | null {
    ...
    for (const group of this.groups) {
      const w =
        Math.max(
          this.$._tile_size.large_w,
          group.width == 0
            ? 0
            : group.width * small_w + (group.width - 1) * tile_gap,
        ) + group_gap;
  ...
  override offset_y_to_y(y: number): { group: string; y: number } | null {
    ...
    const h =
      this.max_height == 0
        ? 0
        : this.max_height * small_h * ((this.max_height - 1) * tile_gap);
  ...
  override forced_offset_x_to_x(
    x: number,
  ): { group: string; x: number } | null {
    let group_x = 0;
    const small_w = this.$._small_size;
    const r = small_w / 2;
    const tile_gap = this.$._tile_gap;
  ...
  override forced_offset_y_to_y(
    y: number,
  ): { group: string; y: number } | null {
      ...
      for (const other_group of this.groups)
        group_x +=
          Math.max(
            ...
            other_group.width == 0
              ? 0
              : other_group.width * small_w +
                  (other_group.width - 1) * tile_gap,
          ) + group_gap;
    ...
    for (const other_group of this.groups) {
      ...
      group_x +=
        Math.max(
          ...
          other_group.width == 0
            ? 0
            : other_group.width * small_w + (other_group.width - 1) * tile_gap,
        ) + group_gap;
    }
    return 0;
  }
    ...
    for (const group of this.groups) {
      ...
      const w =
        Math.max(
          ...
          group.width == 0
            ? 0
            : group.width * this.$._tile_size.small_w +
                (group.width - 1) * this.$._tile_gap,
        ) + this.$._group_gap;
      let tile2c = 0;
      // position each tile
      for (const tile of group.tiles) {
        ...
        const btn_x =
          x + tile.x * this.$._tile_size.small_w + tile.x * this.$._tile_gap;
        const btn_y =
          y + tile.y * this.$._tile_size.small_h + tile.y * this.$._tile_gap;
  ...
  override shift(
    ...
    const group = this.groups.find(
      (g) => !!g.tiles.find((t) => t.id == to_shift),
    )!;
    ...
    switch (place_side) {
      ...
      case "right": {
        ...
        let left_available = false,
          right_available = false;
        if (!(place_taker_w > to_shift_w || place_taker_h > to_shift_h)) {
          if (
            group.is_area_available(
              to_shift_state!.x + place_taker_w,
              to_shift_state!.y,
              to_shift_w,
              to_shift_h,
            ) &&
        ...
        if (place_side == "left") shift_to = right_available ? "right" : null;
        ...
        if (shift_to == "left") {
          ...
          const place_taker_prev_group = this.groups.find(
            (g) => !!g.tiles.find((t) => t.id == place_taker),
          );
          place_taker_state.x = place_taker_new_x;
          place_taker_state.y = to_shift_state!.y;
          place_taker_state.group = group.id;
          const place_taker_tile =
            place_taker_prev_group?.tiles.find((t) => t.id == place_taker) ??
            new Tile(
              place_taker,
              this.$._buttons.get(place_taker)!,
              place_taker_state.x,
              place_taker_state.y,
              get_size_width_small(place_taker_state.size),
              get_size_height_small(place_taker_state.size),
            );
      ...
      if (
        ...
        for (const tile of group.tiles) {
          ...
          if (
            overlap &&
            overlap.area != 0 &&
            tile.button.getAttribute("data-dragging") != "true" &&
            prev.indexOf(tile.id) == -1
          ) {
        ...
        for (const t2 of next_tiles) {
          ...
          new_prev.push(t1);
          this.shift_bottom(t2, t1_h, new_prev, group);
        }
      }
    }
    group.remove(t1);
