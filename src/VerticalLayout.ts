...
export class VerticalLayout extends Layout {
  ...
  override readjust_groups(): void {
    ...
    for (const group of this.groups) {
      ...
      const h =
        Math.max(
          this.$._tile_size.large_h,
          group.height == 0
            ? 0
            : group.height * this.$._tile_size.small_h +
                (group.height - 1) * this.$._tile_gap,
        ) + this.$._group_gap;
      ...
      for (const tile of group.tiles) {
        ...
        const btn_x =
          tile.x * this.$._tile_size.small_w + tile.x * this.$._tile_gap;
        const btn_y =
          y +
          this.$._label_height +
          tile.y * this.$._tile_size.small_h +
          tile.y * this.$._tile_gap;
