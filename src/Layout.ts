...
export class Group {
  ...
  add(tile: Tile): boolean {
    if (
      tile.x < 0 ||
      tile.x + tile.width > this.$.max_width ||
      tile.y < 0 ||
      tile.y + tile.height > this.$.max_height
    )
      return false;
      ...
      a: for (;;) {
        for (const other of this.tiles) {
          ...
          if (overlap && overlap.area != 0) {
            if (tile.y + 1 >= this.$.max_height) tile.x++, (tile.y = 0);
            else tile.y++;
            continue a;
          }
        }
  ...
  auto_self_removal() {
    if (this.tiles.length == 0) this.self_removal();
  }
  ...
  last_tile_position(): [number, number] {
    ...
    t: for (const tile of this.tiles) {
      for (const other of max_horizontal_tiles) {
        if (tile.x > other.x)
          (max_horizontal_tiles.length = 0), max_horizontal_tiles.push(tile);
        else if (tile.x == other.x) max_horizontal_tiles.push(tile);
        else continue t;
      }
    }
    const x =
      max_horizontal_tiles.length == 0
        ? 0
        : max_horizontal_tiles[max_horizontal_tiles.length - 1].x;
    const y = Math.max.apply(
      null,
      max_horizontal_tiles.map((t) => t.y).concat(0),
    );
    return [x, y];
  }

  private _resize(): void {
    this._width = Math.max.apply(
      null,
      this.tiles.map((t) => t.x + t.width).concat(0),
    );
    this._height = Math.max.apply(
      null,
      this.tiles.map((t) => t.y + t.height).concat(0),
    );
  }
}
